...
void handle_reset_graph();  // DODANE: Reset wykresu
...
#define GMC_SEND_INTERVAL       600000  // 6 minut
...
#define WIFI_AP_TIMEOUT         60000   // 1 minuta do przejścia w tryb AP
...
String generateGraphHTML() {
    ...
    function drawHistoryChart() {
        ...
        // UŻYWAMY JEDNEJ SKALI DLA CPM I ACPM
        let maxCPM_ACPM = 0.1;
        let maxUSV = 0.0001;
        ...
        // Podziałki osi Y (lewa strona - CPM/ACPM)
        for (let i = 0; i <= 5; i++) {
        ...
        // Podziałki osi Y (prawa strona - µSv/h)
        for (let i = 0; i <= 5; i++) {
            const y = margin.top + (chartHeight * (5-i) / 5);
            ctx.textAlign = 'left';
        ...
        function drawLine(dataIndex, color, scaleFactor = 1) {
            ...
            // Dla CPM i ACPM używamy maxCPM_ACPM, dla µSv/h używamy maxUSV
        ...
        // Rysujemy linie w odpowiedniej kolejności
        drawLine(0, '#4caf50', 1);        // CPM (zielony)
        const usvScale = maxCPM_ACPM / maxUSV; // Skalowanie µSv/h do skali CPM/ACPM
        drawLine(2, '#ff9800', usvScale); // µSv/h (pomarańczowy) - skalowane
        drawLine(1, '#2196f3', 1);        // ACPM (niebieski) - NA WIERZCHU
        
        ctx.fillStyle = '#58a6ff';
        ctx.font = '12px sans-serif';
        ctx.textAlign = 'center';
        
        // Oznaczenie lewej osi Y
        ctx.save();
...
void update_measurements() {
    ...
    if (now - last_buffer_shift >= 1000) {
        ...
        // Oblicz CPM (2-minutowa średnia) - suma z 120 ostatnich sekund
        uint32_t sum_120 = 0;
        ...
        // Oblicz ACPM (10-minutowa średnia) - suma z 600 ostatnich sekund
        uint32_t sum_600 = 0;
        ...
        // Oblicz µSv/h na podstawie ACPM
...
void init_mdns() {
    ...
    if (!servicesAdded) {
        ...
        MDNS.addServiceTxt("http", "tcp", "version", "0.60a");
...
void connect_wifi() {
    ...
    WiFi.setTxPower(WIFI_POWER_8_5dBm);  // Zmniejszona moc dla lepszej stabilności
...
void start_ap_mode() {
    if (ap_mode) return;  // Jeśli już jesteśmy w trybie AP, nie uruchamiaj ponownie
    
    Serial.println("[WiFi] Przechodzenie w tryb AP...");
    ...
    Serial.println("[AP] Hasło: 12345678");
    digitalWrite(LED_PIN, HIGH);
}
...
void check_wifi_reconnect() {
    ...
    if (!wifi_connected && !ap_mode) {
        // Jeśli nie jesteśmy połączeni i nie jesteśmy w trybie AP
        if (now - wifi_disconnect_time > WIFI_AP_TIMEOUT) {
            // Po 1 minucie bez połączenia przejdź w tryb AP
            Serial.println("[WiFi] Brak połączenia dłużej niż 1 minutę - przechodzę w tryb AP");
            start_ap_mode();
        }
        else if (now - last_wifi_reconnect > WIFI_RECONNECT_INTERVAL) {
            // Próba ponownego połączenia co 30 sekund
        ...
        // Jeśli jesteśmy w trybie AP, ale WiFi się pojawiło, spróbuj ponownie
        if (WiFi.scanNetworks() > 0) {
            String saved_ssid = prefs.getString("ssid", DEFAULT_WIFI_SSID);
            for (int i = 0; i < WiFi.scanComplete(); i++) {
                if (WiFi.SSID(i) == saved_ssid) {
                    Serial.println("[WiFi] Sieć WiFi dostępna - próba ponownego połączenia...");
                    wifi_reconnecting = true;
                    WiFi.mode(WIFI_STA);
...
void handle_root() {
    ...
    html += "<h1>☢ Geiger DIY Monitor v0.60a</h1>";
    ...
    html += "</div>";
    html += "<div style='font-size: 0.9rem; color: #8b949e;'>Wysyłka co 10 minut</div>";
    html += "</div>";
    html += "</div>";
    ...
    // DODANE: Przycisk resetowania wykresu w sekcji diagnostyki
    ...
    html += "<a href='/reset_graph' class='btn btn-warning' style='width: auto; flex: 1; min-width: 200px;' onclick=\"return confirm('Czy na pewno chcesz usunąć całą historię wykresu?\\n\\nWszystkie zebrane dane wykresu zostaną trwale usunięte.\\nNowy wykres rozpocznie się od zera.');\">🗑️ Resetuj wykres</a>";
    ...
    html += "<p>Geiger DIY Monitor v0.60a | Autor: MaxGyver | Hardware: RadiationD v1.1 + ESP32-C3 mini</p>";
...
// DODANE: Funkcja resetująca wykres
void handle_reset_graph() {
    graphInitialize();
    
    String html = "<!DOCTYPE html><html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'>";
    html += "<title>Resetowanie wykresu</title><style>";
    html += "body { font-family: -apple-system, BlinkMacSystemFont, sans-serif; background: #0d1117; color: #c9d1d9; ";
    html += "display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 20px; }";
    html += ".container { background: #161b22; border: 1px solid #30363d; border-radius: 10px; padding: 40px; text-align: center; max-width: 500px; }";
    html += "h1 { color: #2ea043; margin-bottom: 20px; }";
    ...
    html += "<div class='container'>";
    html += "<div class='loader'></div>";
    html += "<h1>🗑️ Wykres zresetowany</h1>";
    html += "<div class='success'>";
    html += "<strong>Sukces!</strong> Wszystkie dane wykresu zostały usunięte.";
    html += "<br><br>Rozpoczęto nowy wykres od początku.";
    html += "<br>Kolejny punkt zostanie dodany za 10 minut.";
    html += "</div>";
    html += "<p>Przekierowanie za 3 sekundy...</p>";
    html += "</div>";
    html += "<script>setTimeout(function() { window.location.href = '/'; }, 3000);</script>";
...
void setup() {
    ...
    server.on("/reset_graph", handle_reset_graph);  // DODANE: Rejestracja nowego endpointu
